...
const { notifyTeam } = require('../services/notificationService');
...
exports.createIncident = async (req, res) => {
  try {
    ...
    // Emit socket event
    req.app.get('io').to(req.user.orgId._id.toString()).emit('incident.created', incident);
...
exports.updateIncident = async (req, res) => {
  try {
    ...
    if (severity && severity !== incident.severity) updates.push(`Severity changed to ${severity}`);
    
    // Only update provided fields
    const fields = { title, description, status, severity, assignedTo, affectedServices };
    ...
    // Emit socket event
    req.app.get('io').to(req.user.orgId._id.toString()).emit('incident.updated', incident);

...
export default function IncidentsList() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-5 max-w-7xl">
      ...
      {/* --- Filter Bar --- */}
      <div className="flex flex-col md:flex-row items-center gap-4 bg-white border-2 border-black p-4 neo-shadow-sm">
        <div className="relative flex-1 w-full">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400" />
          <input
            value={q}
            onChange={(e) => setQ(e.target.value)}
            className="w-full pl-10 pr-4 py-2.5 text-sm bg-zinc-50 border-2 border-black focus:outline-none focus:bg-white transition-all font-bold"
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <Select value={statusF} onValueChange={setStatusF}>
            <SelectTrigger
              className="w-full md:w-48 rounded-none border-2 border-black bg-white font-bold h-[42px]"
